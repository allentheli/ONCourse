...
(async () => {
  ...
  try {
    ...
    // the builder says so before printing; trimming the note brings it back
    {
      ...
      const two = await p3.evaluate(() => { const f = printFit(985, 752, false, 765); return { twoPage: f.twoPage, k: f.k, notice: !document.getElementById('fitnote').hidden }; });
      ...
      ok('the builder warns before printing when a plan needs two pages', two.notice);
      await p3.evaluate(() => { state.opts.notes = ''; renderAll(); }); await p3.waitForTimeout(700);
      const one = await p3.evaluate(() => ({ twoPage: printFit(985, 752, false, 765).twoPage, notice: !document.getElementById('fitnote').hidden }));
      await p3.evaluate(() => applyPrintZoom()); await p3.pdf({ path: f2, preferCSSPageSize: true, landscape: true });
